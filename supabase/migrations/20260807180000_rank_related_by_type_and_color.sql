-- Rank Prop Related Products by the same product type and colour/tone.
--
-- The shared catalog keeps product attributes in products.specs. This
-- migration reads the common colour keys without changing the shared
-- products table. Sequential browsing behaviour remains a strong signal,
-- but visual/type similarity gets priority for cold-start recommendations.
...
as $$
  ...
  candidates as (
    ...
    where pa.id <> current_product_id
      and (
        pa.product_type = cc.product_type
        or pa.collection_group_id = cc.collection_group_id
      )
  ...
  select
    ...
    (
      ...
      + case
          when cardinality(cc.color_tokens) > 0
            and cardinality(c.color_tokens) > 0
            and c.color_tokens && cc.color_tokens
            then 4.0
          else 0
        end
...
comment on function public.get_prop_related_products(bigint, integer) is
  'Prop recommendations ranked by same product type and colour/tone, then browsing sequence, engagement and stock.';
